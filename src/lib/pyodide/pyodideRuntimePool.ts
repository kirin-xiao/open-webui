...
const PERSISTENT_RUNTIME_KEY = '__persistent__';
...
// Tracks which persistence mode the current pool was built under so a runtime
// is never left keyed under the other mode and thus un-terminatable.
let poolPersistent: boolean | null = null;
...
/**
 * Drop the whole pool when the persistence mode changes. Config loads async
 * after boot, so a genuine flip can otherwise strand runtimes created under the
 * previous key scheme. Busy runtimes are terminated once they go idle.
 */
function syncPersistenceMode(): void {
	const mode = persistenceEnabled();
	if (poolPersistent === null) {
		poolPersistent = mode;
		return;
	}
	if (poolPersistent !== mode) {
...
/**
 * Resolve the pool key for a request.
 *
 * With IDBFS persistence enabled the interpreter mounts a single IndexedDB store
 * at `/mnt`; sharing that store across multiple workers is unsafe (a stale
 * `syncfs(false)` reconcile can delete peers' files). So persistence mode keeps
 * one shared runtime, while the default sandbox path isolates per chat.
 */
function poolKeyFor(chatId?: string | null): string {
	return persistenceEnabled() ? PERSISTENT_RUNTIME_KEY : normalizeChatId(chatId);
}
...
/**
 * Terminate and drop a chat's runtime. IDBFS-backed files survive because they
 * live in IndexedDB; in-memory state (variables, imported modules) is discarded.
 *
