...
describe('pyodide runtime pool', () => {
	...
	it('isolates chats even when persistence is enabled (no shared runtime)', () => {
	...
	it('does not dispose pooled runtimes when the persistence mode changes', () => {
		const a = getPyodideRuntime('chat-a').worker;

		config.set({ features: { enable_pyodide_file_persistence: true } } as never);

		// Persistence no longer changes keying, so the existing runtime is reused.
		const b = getPyodideRuntime('chat-a');
		expect(terminated(a)).toBe(0);
		expect(b.worker).toBe(a);
		expect(factory).toHaveBeenCalledTimes(1);
	});

	it('exposes a ready promise that resolves without persistence', async () => {
		const runtime = getPyodideRuntime('chat-a');
		await expect(runtime.ready).resolves.toBeUndefined();
	});

	it('exposes a ready promise that resolves with persistence enabled', async () => {
		config.set({ features: { enable_pyodide_file_persistence: true } } as never);
